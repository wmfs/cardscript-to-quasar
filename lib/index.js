...
module.exports = function (cardscript, options) {
  ...
  function parseElement (element, idx) {
    ...
    if (element.type === 'ActionSet') {
      ...
      if (element.launchesPath) {
        ...
        if (options.ActionStyle === 'dropdown' || options.ActionStyle === 'list') {
          ...
          template += `<q-btn ${loop} ${key} ${onClick} :label="launch.title" unelevated class="q-mb-sm q-mr-sm btn-primary" />`
        }
      } else {
    ...
    if (element.type === 'Input.ApiLookup') {
      ...
      if (element.parametersCard) {
        ...
        template += `<div class="text-right q-mt-md"><q-btn label="${element.buttonText || 'Go'}" unelevated class="btn-primary" :loading="${dataPath}.loading" @click="action('InputApiLookupParamsChanged', ${inspect(input)})" /></div></div>`
        depth--
      }
  ...
  // Validation message
  template += '<div v-if="invalidFields && $v.data.$error" class="bg-negative text-white q-pa-sm" style="border-radius: 4px;">'
  template += '<q-icon name="error_outline" class="q-mr-sm" />'
  template += 'There was a problem processing. Please review any errors above and try again.'
  template += '</div>'
