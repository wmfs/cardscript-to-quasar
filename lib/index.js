  ...
  function parseElement (element, idx) {
    ...
    if (element.type === 'Input.ApiLookup') {
      ...
      if (element.parametersCard) {
        ...
        template += `<q-btn label="${element.buttonText || 'Go'}" unelevated class="q-mb-sm q-mr-sm btn-primary" :loading="${dataPath}.loading" @click="action('InputApiLookupParamsChanged', ${inspect(input)})" />`
