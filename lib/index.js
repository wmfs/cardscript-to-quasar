  ...
  function parseElement (element, idx) {
    ...
    if (element.type === 'Input.ApiLookup') {
      ...
      if (element.parametersCard) {
        ...
        template += `<div class="text-right q-mt-md"><q-btn label="${element.buttonText || 'go'}" color="primary" :loading="data.${element.id}.loading" @click="action('InputApiLookupParamsChanged', ${inspect(element)})" /></div></div>`
        depth--
      }
      parseElement({
        ...
        showLaunches: element.showLaunches,
        clickToLaunch: element.clickToLaunch,
        showPagination: element.showPagination,
