  ...
  function parseElement (element, idx) {
    ...
    if (element.type === 'Input.ApiLookup') {
      ...
      if (element.parametersCard) {
        ...
        template += `<div class="text-right q-mt-md"><q-btn label="go" color="primary" @click="action('InputApiLookupParamsChanged', ${inspect(element)})" /></div></div>`
        depth--
      }
      parseElement({ type: 'EndInput.ApiLookup', id: element.id, templates: element.templates })
      depth--
    }
  }
