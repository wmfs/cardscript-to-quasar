module.exports = function applyQa (config, source) {
  ...
  const qChatDiv = div
    .addChildTag('div')
    .addAttribute('v-for', `(message, idx) in ${dataPath}.$QA.${config.id}.qaComments`)
  ...
  const responseInput = responseInputDiv
    .addChildTag('q-input')
    .addAttribute('v-model', `${dataPath}.$QA.${config.id}.qaComments[idx].responseInput`)
  ...
  const input = div
    .addChildTag('q-input')
    .addAttribute('v-model', `${dataPath}.$QA.${config.id}.qaComment.comment`)
  ...
  const commentInput = div
    .addChildTag('div')
    .addChildTag('div').content('QA Comments')
    .addChildTag('q-input')
    .addAttribute('v-model', `${dataPath}.$QA.comments`)
    .addAttribute('class', 'q-py-md')

  commentInput.addAttribute('type', 'textarea')

  div.addAttribute('v-if', `${dataPath}.$QA.${config.id}.toggleQa`)
}
