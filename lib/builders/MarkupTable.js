...
function vertical (qMarkupTable, definition) {
  ...
  for (const { title, value, showWhen, badge, tooltip, showLaunches, titleTooltip } of columns) {
    const tr = tBody.addChildTag('tr')

    if (showWhen) tr.addAttribute('v-if', showWhen)

    if (!hideHeader) {
      applyVerticalHeader(tr, { title, titleTooltip })
    }
...
function applyVerticalHeader (tr, { title, titleTooltip }) {
  const titleTd = tr.addChildTag('td')
