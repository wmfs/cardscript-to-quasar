...
function horizontal (qMarkupTable, definition) {
  ...
  for (const { id, value, showWhen, badge, tooltip, showLaunches, editable } of columns) {
    const cell = tBodyRow
      .addChildTag('td')
      .addAttribute('class', 'text-left')
    ...
    if (editable && id) applyEditable(cell, id)
...
function vertical (qMarkupTable, definition) {
  ...
  for (const { id, title, value, showWhen, badge, tooltip, showLaunches, titleTooltip, color, editable } of columns) {
...
function applyEditable (cell, id) {
  cell
    .addChildTag('q-popup-edit')
    .addAttribute('v-model', `item.${id}`)
