...
module.exports = function (definition, options) {
  ...
  if (fixedHeader) qMarkupTable.addAttribute('style', 'overflow: auto; height: 75vh; border: 1px solid #EEE;')
...
function horizontal (qMarkupTable, definition) {
  ...
  if (!hideHeader) {
    ...
    if (selectionType) {
      const selectAllTh = tHeadRow.addChildTag('th')

      selectAll(selectAllTh, definition)
    }
  ...
  for (const { value, showWhen, badge, tooltip, showLaunches, editable } of columns) {
    const cell = tBodyRow
      .addChildTag('td')
      .addAttribute('class', 'text-left')
      .addAttribute('tabindex', '0')
    ...
    if (editable) applyEditable(cell, editable)
...
function vertical (qMarkupTable, definition) {
  ...
  if (selectionType) {
    ...
    if (!hideHeader) {
      const selectAllTd = tr.addChildTag('td').addAttribute('tabindex', '0')

      selectAll(selectAllTd, definition)
    }
  ...
  for (const { title, value, showWhen, badge, tooltip, showLaunches, titleTooltip, color, editable } of columns) {
...
function applyLaunches (td, definition) {
  const { removableRows, removeInLaunches, removeText, arrayPath } = definition
...
function selectAll (td, definition) {
  const {
    id,
    selectionType,
    selectionMax = 0,
  ...
  if (selectionType === 'multi' && selectionMax === 0) {
    const destPath = `${td.getDataPath()}.${id}Selected`

    td
      .addChildTag('q-checkbox')
      .addAttribute(':value', `${arrayPath}.every(row => !!${destPath}.find(r => hashSum(r) === hashSum(row)))`)
      .addAttribute('@input', `action('ListSelectAll', ${inspect({ destPath: `${destPath}`, sourcePath: arrayPath })})`)
  }
} // selectAll

function applyEditable (cell, editable) {
  const { id, type } = editable

  if (!['Input.Text', 'Input.Number'].includes(type)) return

  const dataPath = `item.${id}`

  const popupEdit = cell
    .addChildTag('q-popup-edit')
    .addAttribute('v-model', dataPath)
    .addAttribute('persistent', null)
    .addAttribute('buttons', null)
    // todo :validate

  const input = popupEdit
    .addChildTag('q-input')
    .addAttribute('dense', null)
    .addAttribute('autofocus', null)

  if (type === 'Input.Text') {
    input.addAttribute('v-model', dataPath)
  }
