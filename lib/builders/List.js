...
const { TEXT_WHITE } = require('../utils/color-reference')
...
function renderCards (parent, definition) {
  ...
  if (selectionClearable && selectionType === 'multi') {
    applyClearSelection(parent.addChildTag('div'), definition)
  }

  if (validation) {
    applyValidation(parent.addChildTag('div'), definition)
  }
} // renderCards
...
function applyAvatar (item, definition) {
  ...
  if (avatar) {
    ...
    for (const a of avatars) {
      const { type, source, sourcePath, color, showWhen } = a
      ...
      if (['icon', 'letter', 'image'].includes(type) && (source || sourcePath)) {
        ...
        if (type === 'icon') {
          ...
          if (showWhen) icon.addAttribute('v-if', showWhen)
          if (source) icon.addAttribute('name', source)
          if (sourcePath) icon.addAttribute(':name', sourcePath)
        }

        if (type === 'letter') {
          const letter = itemSection
            .addChildTag('q-avatar')

          if (color && COLORS[color]) {
            letter.addAttribute('color', COLORS[color])
            if (TEXT_WHITE.includes(color)) {
              letter.addAttribute('text-color', 'white')
            }
          } else {
            letter.addAttribute('color', COLORS.accent)
            letter.addAttribute('text-color', 'white')
          }

          if (showWhen) letter.addAttribute('v-if', showWhen)
          if (source) letter.content(source)
          if (sourcePath) letter.content(`{{ ${sourcePath} }}`)
        }

        if (type === 'image') {
          const imageParent = itemSection.addChildTag('q-avatar')
          if (showWhen) imageParent.addAttribute('v-if', showWhen)

          const image = imageParent.addChildTag('img', { isSelfClosingTag: true })
