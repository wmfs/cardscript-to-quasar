...
const stringTemplateParser = require('../utils/string-template-parser')
...
function renderList (builder, definition) {
  ...
  if (templates) {
    ...
    const itemSection = item.addChildTag('q-item-section') // center section
    ...
    if (stamp) {
      item
        .addChildTag('q-item-section') // right section
...
function renderCards (builder, definition) {
  ...
  if (imgSrcPath) card.addChildTag('img').addAttribute('alt', 'This is an image without alt text').addAttribute(':src', imgSrcPath)
...
function applyLaunches (parent, definition) {
  ...
  const btn = section
    ...
    .addAttribute('aria-label', 'more options')
...
function applySelection (parent, definition) {
  ...
  option.addAttribute(':aria-label', stringTemplateParser(definition.templates.label))

  const destPath = `${parent.getDataPath()}.${id}`

  if (selectionType === 'multi') {
    option.addAttribute(':value', rowKey ? `${destPath}.includes(item.${rowKey})` : `!!${destPath}.find(r => hashSum(r) === hashSum(item))`)
  } else if (selectionType === 'single') {
    option.addAttribute(':val', rowKey ? `item.${rowKey}` : 'item')
    option.addAttribute(':value', `${destPath}`)
  }

  if (onSelectionEndpoint) {
    option.addAttribute('@input', `action('ListSelection', { idx, sourcePath: '${arrayPath}', destPath: '${destPath}', selectionMax: ${selectionMax}, selectionType: '${selectionType}', rowKey: '${rowKey}', onSelectionEndpoint: ${inspect(onSelectionEndpoint)} })`)
  } else {
    option.addAttribute('@input', `action('ListSelection', { idx, sourcePath: '${arrayPath}', destPath: '${destPath}', selectionMax: ${selectionMax}, selectionType: '${selectionType}', rowKey: '${rowKey}' })`)
  }
} // applySelection
...
function selectAll (builder, definition) {
  ...
  if (selectionType === 'multi' && selectionMax === 0) {
    ...
    item
      ...
      .addAttribute(':value', `${arrayPath}.every(row => ${rowKey ? `${destPath}.includes(row.${rowKey})` : `!!${destPath}.find(r => hashSum(r) === hashSum(row))`})`)
      .addAttribute('@input', `action('ListSelectAll', ${inspect({ destPath: `${destPath}`, sourcePath: arrayPath, rowKey })})`)
