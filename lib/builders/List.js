...
function renderCards (builder, definition) {
  ...
  if (templates) {
    ...
    if (badge) applyBadges(badge, badgeColor, card, listStyle)
  ...
  if (showLaunches) {
    ...
    const btn = divColActions
      .addChildTag('q-btn')
      .addAttribute('v-if', 'item.launches && item.launches.length')
...
function renderList (builder, definition) {
  ...
  if (selectionType === 'multi' && selectionMax === 0) {
    ...
    selectAllItem
      ...
      .addAttribute(':value', `${arrayPath}.every(row => ${rowKey ? `${list.getDataPath()}.${id}.includes(row.${rowKey})` : `!!${list.getDataPath()}.${id}.find(r => hashSum(r) === hashSum(row))`})`)
      .addAttribute('@input', `action('ListSelectAll', ${inspect({ destPath: `${list.getDataPath()}.${id}`, sourcePath: arrayPath, rowKey })})`)
  ...
  if (selectionType === 'single' || selectionType === 'multi') {
    ...
    const destPath = `${list.getDataPath()}.${id}`
    ...
    if (selectionType === 'multi') {
      list
        ...
        .addAttribute('@click', `action('ClearArraySelection', '${list.getDataPath()}.${id}')`)
  ...
  if (templates) {
    ...
    if (badge) applyBadges(badge, badgeColor, itemMain, listStyle)
...
function applyBadges (badge, badgeColor, ele, listStyle) {
  const badges = Array.isArray(badge) ? badge : [{ text: badge, colour: badgeColor || 'accent' }]
  const badgeItemLabel = badges.length
    ? ele.addChildTag(listStyle === 'list' ? 'q-item-label' : 'q-card-section')
    : null
  ...
  for (const b of badges) {
    ...
    if (b.icon) badge.addChildTag('q-icon').addAttribute('name', b.icon)
