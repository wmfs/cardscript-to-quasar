...
module.exports = async function ActionTemplate (definition, elementOptions, additionalOptions) {
  const {
    ActionSet,
    ActionSetDropDown,
    checkStateMachinePermissions
  } = elementOptions
  ...
  if (type === 'Action.PushCard' && checkStateMachinePermissions) {
    const allowed = await checkStateMachinePermissions(stateMachineName)
    if (!allowed) {
      console.log('User cannot access', stateMachineName)
