...
module.exports = function (definition, options) {
  ...
  if (theme === 'GDS') {
    ...
    for (const key of ['date', 'month', 'year']) {
      ...
      row
        ...
        .addAttribute('@input', `val => action('UpdateGDSDate', { dataPath: '${dataPath}', value: val, key: '${key}', min: '${min}', max: '${max}' })`)
    ...
    let errorMessages = `{{ ${dataPath}_$DATE.errors.join(' ') }}`
    if (id) {
      errorMessages += ` {{ invalidFields && invalidFields['${id}'] ? invalidFields['${id}'].messages.join(' ') : '' }}`
    }
  ...
  if (clearable) {
    input
      ...
      .addAttribute('class', 'cursor-pointer')
      .addAttribute('@click.native', `setNullAtDataPath('${input.getDataPath() + '.' + id}')`)
