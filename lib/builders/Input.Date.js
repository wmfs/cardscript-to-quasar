...
function prependTemplate (field, definition) {
  ...
  const dateEl = popupProxy
    ...
    .addAttribute('@update:model-value', `val => action('DateEntry', { val, dataPath: '${dataPath}', endpoint: ${endpoint ? inspect(endpoint) : null}, resetDataPaths: ${Array.isArray(resetDataPaths) && resetDataPaths.length > 0 ? inspect(resetDataPaths) : null} })`)
