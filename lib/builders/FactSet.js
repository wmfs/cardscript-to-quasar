...
module.exports = function (definition, options) {
  const { showIcons } = definition
  const builder = new ComponentBuilder(definition)

  const list = builder.addTag('q-list')
  list.addAttribute('class', 'non-link-item')

  definition.facts.forEach(({ title, value, icon, inset, choices, showIfUndefined, showWhen }) => {
...
function lookupOrValue (title, value, choices) {
  ...
  return `{{ lists.$simpleTitleMaps['${title}'][${data}] || ${data} || '-' }}`
}
