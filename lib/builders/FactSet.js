  ...
  if (Array.isArray(facts)) {
    for (const fact of facts) {
      ...
      if (Array.isArray(choices)) {
        ...
        if (tooltip) {
          applyTooltip({ source: arraySpan, tooltip })
        }
        ...
        const valueItemLabel1 = itemMainSection
          .addChildTag('q-item-label')
        ...
        const valueItemLabel2 = itemMainSection
          .addChildTag('q-item-label')
          .addAttribute('v-if', `!Array.isArray(${dataPath}) && Array.isArray(${choicesPath})`)
          .addChildTag('span')
        ...
        if (tooltip) {
          applyTooltip({ source: valueItemLabel1, tooltip })
          applyTooltip({ source: valueItemLabel2, tooltip })
        }
      } else {
        const valueItemLabel = itemMainSection
          .addChildTag('q-item-label')
