...
module.exports = function (definition, options) {
  ...
  if (actionStyle === 'buttons') {
    ...
    if (actionStyle === 'dropdown') {
      ...
      if (title) {
        parent.addAttribute('label', title)
      } else {
        parent.addAttribute('rounded', null)
        parent.addAttribute('dense', null)
      }

      if (showWhen) {
        parent.addAttribute('v-if', showWhen)
      }
    }
    ...
    if (actionStyle === 'list') {
      ...
      if (showWhen) {
        listGroup.addAttribute('v-if', showWhen)
      }
    }
  }
