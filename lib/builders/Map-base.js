...
function addMarkerOSGridRef (idx, m, markerDetails, map, builder, definition) {
  if (markerDetails) {
    ...
    markerLabel(m, item, definition, 'OSGridRef')

    if (definition.type === 'Input.Map') {
      markerEditButton(idx, m, item, map, builder, 'OSGridRef')
    }
  }
...
function addMarkerLongLat (idx, m, markerDetails, map, builder, definition) {
  if (markerDetails) {
    ...
    markerLabel(m, item, definition, 'LatLon')

    if (definition.type === 'Input.Map') {
      markerEditButton(idx, m, item, map, builder, 'LatLon')
    }
  }
...
function markerEditButton (idx, m, item, map, builder, format) {
  ...
  item
    ...
    .addAttribute('flat', null)
    .addAttribute('dense', null)
    .addAttribute('@click', `prompt${suffix}Open('${map.getDataPath()}.MAP_PROMPT_${idx}', data.${m[xBinding]}, data.${m[yBinding]})`)
  ...
  dialogCardActions
    ...
    .addAttribute('@click', `prompt${suffix}Submit('${map.getDataPath()}.MAP_PROMPT_${idx}', '${map.getDataPath()}.${m[xBinding]}', '${map.getDataPath()}.${m[yBinding]}')`)

  dialogCardActions
    .addChildTag('q-btn')
    .addAttribute('flat', null)
    .addAttribute('label', 'Default to geolocation')
    .addAttribute('@click', `prompt${suffix}Submit('${map.getDataPath()}.MAP_PROMPT_${idx}', '${map.getDataPath()}.${m[xBinding]}', '${map.getDataPath()}.${m[yBinding]}', true)`)
} // markerEditButton
