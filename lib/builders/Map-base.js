...
function MapBuilder (definition, options) {
  ...
  map.addAttribute('aria-label', definition.type === 'Map' ? 'A non-interactive map' : 'An interactive map')
...
function markerLabel (m, item, definition, format) {
  ...
  // Set m.showCoordinates to true by default
  if (m.showCoordinates === undefined) m.showCoordinates = true

  if (m.showCoordinates) {
    label
...
function mapCircle (map, m, format) {
  ...
  const circle = map
    ...
    .addAttribute(':showLaunches', m.showLaunches || 'false')
    .addAttribute(':launches', m.launches || '[]')
...
function addMarkerPath (map, definition, markerDetails) {
  ...
  const circle = map
    ...
    .addAttribute(':showLaunches', 'm.showLaunches || false')
    .addAttribute(':launches', 'm.launches || []')
    .addAttribute('@launch', 'launch => mapMarkerLaunch(launch)')
  ...
  if (markerDetails) {
    ...
    label
      ...
      .addAttribute('v-if', 'm.format === \'OSGridRef\' && m.showCoordinates')
      .content('{{ m.x }}, {{ m.y }}')

    label
      .addChildTag('q-item-label')
      .addAttribute('caption', null)
      .addAttribute('v-if', 'm.format === \'LatLon\' && m.showCoordinates')
    ...
    addMarkerPathLaunches(item)
  }
} // addMarkerPath

function addMarkerPathLaunches (item) {
  const section = item
    .addChildTag('q-item-section')
    .addAttribute('side', null)
    .addAttribute('top', null)
    .addAttribute('m.showLaunches && m.launches && m.launches.length > 0')
  ...
  list
    ...
    .addAttribute('v-for', '(launch, launchIdx) in m.launches')
