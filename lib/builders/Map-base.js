...
function MapBuilder (definition, options) {
  ...
  const detailsCard = (definition.showMarkerLabels || definition.showCentreLabels)
    ? builder.addTag('q-card').addChildTag('q-card-section')
    : null
  ...
  const markerList = (detailsCard && markers.length > 0)
    ? detailsCard.addChildTag('q-list')
    : null

  for (const [idx, m] of markers.entries()) {
    if (m.format === 'OSGridRef') {
      addOSGridRefMarker(idx, m, map, markerList, definition, builder)
    } else {
      addLatLongMarker(idx, m, map, markerList, definition, builder)
    }
  }

  addMarkersArray(map, definition)

  return { builder, map }
} // MapBuilder

function addOSGridRefMarker (idx, m, map, markerList, definition, builder) {
  if (markerList) {
  ...
  addMapMarker(m, 'OSGridRef', map)
} // addOSGridRefMarker

function addLatLongMarker (idx, m, map, markerList, definition, builder) {
  if (markerList) {
    const item = markerList.addChildTag('q-item')
  ...
  addMapMarker(m, 'LatLong', map)
} // addLatLongMarker

function addMapMarker (m, format, map) {
  const xCoord = (format === 'OSGridRef') ? 'x' : 'longitude'
  const yCoord = (format === 'OSGridRef') ? 'y' : 'latitude'

  const xBinding = `data.${m[xCoord]}`
  const yBinding = `data.${m[yCoord]}`

  const marker = map
    .addChildTag('q-map-circle')
    .addAttribute('format', format)
    .addAttribute(`:${xCoord}`, xBinding)
    .addAttribute(`:${yCoord}`, yBinding)
  ...
  if (!m.locked) {
    marker
      .addAttribute(`@${xCoord}`, `v => { ${xBinding} = v }`)
      .addAttribute(`@${xCoord}`, `v => { ${yBinding} = v }`)
  }
} // addMapMarker

function addMarkersArray (map, definition) {
  if (definition.markersArrayPath) {
    map
      .addChildTag('q-map-circle')
