...
function MapBuilder (definition, options) {
  ...
  for (const [idx, m] of markers.entries()) {
    if (m.format === 'OSGridRef') {
      addOSGridRefMarker(m, map, markerList)
    } else {
      addLatLongMarker(m, map, markerList)
    }
  }
...
function addOSGridRefMarker (m, map, markerList) {
  if (markerList) {
...
function addLatLongMarker (m, map, markerList) {
  if (markerList) {
    const item = markerList.addChildTag('q-item')
