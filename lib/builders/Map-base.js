...
function MapBuilder (definition, options) {
  ...
  for (const [idx, m] of markers.entries()) {
    ...
    markerFn(idx, m, markerList, map, builder, definition)
  } // for ...
...
function addMarkerOSGridRef (idx, m, markerList, map, builder, definition) {
  if (markerList) {
...
function addMarkerLongLat (idx, m, markerList, map, builder, definition) {
  if (markerList) {
    const item = markerList.addChildTag('q-item')
