...
module.exports = function (definition, options) {
  ...
  if (selectionType) {
    bodyTr
      ...
      .addAttribute(':value', `!!data.${id}Selected.find(r => r.rowKey === props.row.rowKey)`)
      .addAttribute('@input', `action('AddTableRow', { row: props.row, arrayPath: 'data.${id}Selected', selectionType: '${selectionType}' })`)
  }
  ...
  for (const [idx, col] of columns.entries()) {
    ...
    if (col.editable) {
      if (['Text', 'Number', 'Toggle'].includes(col.inputType)) {
        const qPopupEdit = td
          .addChildTag('q-popup-edit')
          .addAttribute('v-model', `props.row.${col.field}`)
          .addAttribute('title', `Update ${col.title}`)
          .addAttribute('buttons', null)

        switch (col.inputType) {
          case 'Text': {
            const qInput = qPopupEdit.addChildTag('q-input')
              .addAttribute('v-model', `props.row.${col.field}`)
              .addAttribute('dense', null)
              .addAttribute('autofocus', null)

            if (col.inputPlaceholder) qInput.addAttribute('placeholder', col.inputPlaceholder)
            break
          }
          case 'Number': {
            const qInput = qPopupEdit.addChildTag('q-input')
              .addAttribute('type', 'number')
              .addAttribute('@input', `val => props.row.${col.field} = val === '' ? null : +val`)
              .addAttribute(':value', `props.row.${col.field}`)
              .addAttribute('dense', null)
              .addAttribute('autofocus', null)
          ...
          case 'Toggle': {
            qPopupEdit.addChildTag('q-toggle')
              .addAttribute('v-model', `props.row.${col.field}`)
          ...
          // todo: Other inputTypes (follow the Input. components) Text, Number, Toggle
          // todo: validation?
        }
      }
    }
