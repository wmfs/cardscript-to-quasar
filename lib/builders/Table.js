...
module.exports = function (definition, options) {
  ...
  const _columns = `[${columns.map(({ title, field, format, sortable }) => {
    const attr = [
      `name: 'col-${colIdx}'`,
      `label: '${title}'`,
      `field: '${field}'`,
    ...
    if (sortable) attr.push(`sortable: true`)
  ...
  table.addAttribute('row-key', rowKey || 'name')
