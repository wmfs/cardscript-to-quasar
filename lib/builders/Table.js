...
module.exports = function (definition, options) {
  ...
  bottomTemplate(table, arrayPath)
...
function bottomTemplate (table, arrayPath) {
  const template = table
    .addChildTag('template')
    .addAttribute('v-slot:bottom', 'props')
  ...
  const totalHits = `${arrayPath}.length`

  div
    .addChildTag('q-pagination')
    .addAttribute('v-if', 'props.pagesNumber > 1')
    .addAttribute('v-model', page)
    .addAttribute(':min', 1)
    .addAttribute(':max-pages', 4)
    .addAttribute(':max', 'props.pagesNumber')
  ...
  div
    .addChildTag('div')
    .addAttribute('v-if', `${totalHits} === 1`)
    .addAttribute('class', 'text-weight-light q-mx-md')
    .content(`Showing 1 result`)
}
