...
function onTextInput (element, definition) {
  ...
  element.addAttribute('@update:model-value', `val => action('TextInput', { val, dataPath: '${dataPath}', resetDataPaths: ${Array.isArray(resetDataPaths) && resetDataPaths.length > 0 ? inspect(resetDataPaths) : null}, launchOnChange: ${launchOnChange ? inspect(launchOnChange) : null} })`)

  if (launchOnChange) {
    element.addAttribute(':loading', `${dataPath}Loading`)
    element.addAttribute(':disable', `${dataPath}Loading`)
  }
}
