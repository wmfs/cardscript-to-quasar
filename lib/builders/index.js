module.exports = {
  AdaptiveCard: { conversionFunction: require('./AdaptiveCard') },
  Separator: { conversionFunction: require('./Separator') },
  ActionSet: { conversionFunction: require('./ActionSet') },
  Table: { conversionFunction: require('./Table') },
  List: { conversionFunction: require('./List') },
  TabSet: { conversionFunction: require('./TabSet') },
  Tab: { conversionFunction: require('./Tab') },
  CardList: { conversionFunction: require('./CardList') },
  EndCardList: { conversionFunction: require('./EndCardList') },
  'EndInput.ApiLookup': { conversionFunction: require('./EndInput.ApiLookup') },
  Chip: { conversionFunction: require('./Chip') },
  Icon: { conversionFunction: require('./Icon') },
  Collapsible: { conversionFunction: require('./Collapsible') },
  Container: { conversionFunction: require('./Container') },
  TextBlock: { conversionFunction: require('./TextBlock') },
  Image: { conversionFunction: require('./Image') },
  Map: { conversionFunction: require('./Map') },
  Graph: { conversionFunction: require('./Graph') },
  AddressBlock: { conversionFunction: require('./AddressBlock') },
  PhaseBanner: { conversionFunction: require('./PhaseBanner') },
  Media: { conversionFunction: require('./Media') },
  MediaSource: { conversionFunction: require('./MediaSource') },
  ColumnSet: { conversionFunction: require('./ColumnSet') },
  Column: { conversionFunction: require('./Column') },
  FactSet: { conversionFunction: require('./FactSet') },
  ImageSet: { conversionFunction: require('./ImageSet') },
  Jumbotron: { conversionFunction: require('./Jumbotron') },
  Timeline: { conversionFunction: require('./Timeline') },
  TimelineEntry: { conversionFunction: require('./TimelineEntry') },
  Pagination: { conversionFunction: require('./Pagination') },
  'Action.CopyToClipboard': { conversionFunction: require('./Action-template') },
  'Action.ApiLookup': { conversionFunction: require('./Action-template') },
  'Action.OpenUrl': { conversionFunction: require('./Action-template') },
  'Action.Submit': { conversionFunction: require('./Action-template') },
  'Action.ShowCard': { conversionFunction: require('./Action-template') },
  'Action.PushCard': { conversionFunction: require('./Action-template') },
  'Action.ReplaceCard': { conversionFunction: require('./Action-template') },
  'Action.ClearArraySelection': { conversionFunction: require('./Action-template') },
  'Action.Cancel': { conversionFunction: require('./Action-template') },
  'Action.NextTab': { conversionFunction: require('./Action-template') },
  'Action.PreviousTab': { conversionFunction: require('./Action-template') },
  'Action.Save': { conversionFunction: require('./Action-template') },
  'Action.Stop': { conversionFunction: require('./Action-template') },
  'Input.Text': { conversionFunction: require('./Input.Text') },
  'Input.Number': { conversionFunction: require('./Input.Number') },
  'Input.Date': { conversionFunction: require('./Input.Date') },
  'Input.DateTime': { conversionFunction: require('./Input.DateTime') },
  'Input.Time': { conversionFunction: require('./Input.Time') },
  'Input.Toggle': { conversionFunction: require('./Input.Toggle') },
  'Input.ChoiceSet': { conversionFunction: require('./Input.ChoiceSet') },
  'Input.Slider': { conversionFunction: require('./Input.Slider') },
  'Input.Address': { conversionFunction: require('./Input.Address') },
  'Input.ApiLookup': { conversionFunction: require('./Input.ApiLookup') },
  'Input.Currency': { conversionFunction: require('./Input.Currency') },
  'Input.Email': { conversionFunction: require('./Input.Email') },
  'Input.FileUpload': { conversionFunction: require('./Input.FileUpload') },
  'Input.Gender': { conversionFunction: require('./Input.Gender') },
  'Input.Name': { conversionFunction: require('./Input.Name') },
  'Input.Signature': { conversionFunction: require('./Input.Signature') },
  'Input.TelephoneNumber': { conversionFunction: require('./Input.TelephoneNumber') },
  'Input.Map': { conversionFunction: require('./Input.Map') },
  'Input.Chat': { conversionFunction: require('./Input.Chat') },
  Tree: { conversionFunction: require('./Tree') },
  LinearProgressBar: { conversionFunction: require('./LinearProgressBar') },
  EndAdaptiveCard: { conversionFunction: require('./EndAdaptiveCard') },
  MarkupTable: { conversionFunction: require('./MarkupTable') },
  Stepper: { conversionFunction: require('./Stepper') },
  Step: { conversionFunction: require('./Step') }
}
