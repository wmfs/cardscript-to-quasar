...
module.exports = function (definition, options) {
  ...
  if (selectionType === 'Select') {
    ...
    if (filter) {
      select
        .addAttribute('@filter', `(val, update) => filterChoiceSetFn('${id}', val, update, '${definition.choicesPath || ''}')`)
    ...
    if (id && validation) {
      ...
      applyErrorCheck(select, definition)
    }
    ...
    if (title) {
      labelDiv
        .addChildTag('span')
        .addAttribute(':class', `data.$HAS_VALIDATED && $v.data.${id} && $v.data.${id}.$error ? 'label-error' : ''`)
        .content(title)
    }
    ...
    const templateControl = field
      .addChildTag('template')
      .addAttribute('v-slot:control', null)

    const opt = templateControl
      .addChildTag('div')
    ...
    if (id && validation) {
      ...
      applyErrorCheck(field, definition)
    }
  }

  div.addAttribute('class', outerDivClasses.join(' '))
  div.addAttribute('aria-label', title)
